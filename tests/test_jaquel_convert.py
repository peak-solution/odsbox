...
def test_predefined():
    ...
    for path in Path(predefined_path).rglob("*.json"):
        ...
        jaquel_result = jaquel_to_ods_ex(model, jaquel_dict)
...
def test_jaquel_to_ods_ex():
    model = __get_model("application_model.json")

    jaquel_result = jaquel_to_ods_ex(model, {"AoMeasurement": 1})
...
def test_jaquel_to_ods_ex_special():
    model = __get_model("application_model.json")

    jaquel_result = jaquel_to_ods_ex(
        model,
        {
            "AoMeasurement": 1,
            "$attributes": {
...
def test_jaquel_to_ods_ex_aggregate():
    model = __get_model("application_model.json")

    jaquel_result = jaquel_to_ods_ex(model, {"AoMeasurement": 1, "$attributes": {"id": {"$min": 1}}})
    assert jaquel_result is not None
    assert jaquel_result.entity.name == "MeaResult"
    assert jaquel_result.select_statement is not None
